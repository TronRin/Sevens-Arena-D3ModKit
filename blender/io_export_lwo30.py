...
bpy.types.Material.vcmenu: bpy.props.EnumProperty(
...
class PANEL_PT_idTechVertexColors(bpy.types.Panel):
...
class MESSAGE_OPERATOR_Message(bpy.types.Operator):
...
class EXPORT_OT_LwoExport(bpy.types.Operator, ExportHelper):
			...
			if self.option_applymod:
				if not(objdup.data.shape_keys):
					while (len(objdup.modifiers)):
						bpy.ops.object.modifier_apply(modifier = objdup.modifiers[0].name)
...
def menu_func(self, context):
	self.layout.operator(EXPORT_OT_LwoExport.bl_idname, text="Lightwave (.lwo)")


classes = (
	PANEL_PT_idTechVertexColors,
	MESSAGE_OPERATOR_Message,
	EXPORT_OT_LwoExport,
)
