...
void idPlayerView::ClearEffects() {
	lastDamageTime = MS2SEC( GAME_TIME - 99999 );

	dvFinishTime = ( GAME_TIME_FAST - 99999 );
	kickFinishTime = ( GAME_TIME - 99999 );

	for ( int i = 0 ; i < MAX_SCREEN_BLOBS ; i++ ) {
		screenBlobs[i].finishTime = GAME_TIME;
	}
...
void idPlayerView::DamageImpulse( idVec3 localKickDir, const idDict *damageDef ) {
	if ( g_hitEffect.GetBool() ) {
		...
		//
		if ( lastDamageTime > 0.0f && SEC2MS( lastDamageTime ) + IMPULSE_DELAY > GAME_TIME ) {
		...
		if ( dvTime ) {
			if ( dvFinishTime < GAME_TIME_FAST ) {
				dvFinishTime = GAME_TIME_FAST;
			}
			dvFinishTime += g_dvTime.GetFloat() * dvTime;
			// don't let it add up too much in god mode
			if ( dvFinishTime > GAME_TIME_FAST + 5000 ) {
				dvFinishTime = GAME_TIME_FAST + 5000;
			}
		}
		...
		if ( kickTime ) {
			kickFinishTime = GAME_TIME + g_kickTime.GetFloat() * kickTime;
		...
		if ( blobTime ) {
			...
			blob->startFadeTime = GAME_TIME;
			blob->finishTime = GAME_TIME + blobTime * g_blobTime.GetFloat() * ( ( float )gameLocal.msec / USERCMD_MSEC );
		...
		//
		lastDamageTime = MS2SEC( GAME_TIME );
	}
}
...
/*
	...
	screenBlob_t *blob = GetScreenBlob();
	blob->startFadeTime = GAME_TIME;
	blob->finishTime = GAME_TIME + ( duration * 1000 );
...
void idPlayerView::WeaponFireFeedback( const idDict *weaponDef ) {
	...
	if ( recoilTime && kickFinishTime < GAME_TIME ) {
		idAngles angles;
		weaponDef->GetAngles( "recoilAngles", "5 0 0", angles );
		kickAngles = angles;
		int	finish = GAME_TIME + g_kickTime.GetFloat() * recoilTime;
...
void idPlayerView::CalculateShake() {
	...
	float shakeVolume = gameSoundWorld->CurrentShakeAmplitudeForPosition( GAME_TIME, player->firstPersonViewOrigin );
	//
...
idAngles idPlayerView::AngleOffset() const {
	...
	if ( GAME_TIME < kickFinishTime ) {
		float offset = kickFinishTime - GAME_TIME;
...
void idPlayerView::SingleView( idUserInterface *hud, const renderView_t *view ) {
	...
	gameSoundWorld->PlaceListener( view->vieworg, view->viewaxis, player->entityNumber + 1, GAME_TIME, hud ? hud->State().GetString( "location" ) : "Undefined" );

	// if the objective system is up, don't do normal drawing
	if ( player->objectiveSystemOpen ) {
		player->objectiveSystem->Redraw( GAME_TIME_FAST );
		return;
	}
	...
	if ( !pm_thirdPerson.GetBool() && !g_skipViewEffects.GetBool() ) {
		for ( int i = 0 ; i < MAX_SCREEN_BLOBS ; i++ ) {
			...
			if ( blob->finishTime <= GAME_TIME ) {
				continue;
			}

			blob->y += blob->driftAmount;

			float	fade = (float)( blob->finishTime - GAME_TIME ) / ( blob->finishTime - blob->startFadeTime );
			if ( fade > 1.0f ) {
				fade = 1.0f;
			}
			if ( fade ) {
		...
		float	armorPulse = ( GAME_TIME_FAST - player->lastArmorPulse ) / 250.0f;
		...
		if ( alpha < 1.0f  ) {
			renderSystem->SetColor4( ( player->health <= 0.0f ) ? MS2SEC( GAME_TIME ) : lastDamageTime, 1.0f, 1.0f, ( player->health <= 0.0f ) ? 0.0f : alpha );
...
bool FxFader::SetTriggerState( bool active ) {
	...
	switch ( state ) {
		...
		case FX_STATE_RAMPUP:	alpha = 1 - (float)( time - GAME_TIME ) / msec; break;
		case FX_STATE_RAMPDOWN:	alpha = (float)( time - GAME_TIME ) / msec; break;
	}
...
void FullscreenFX_Warp::HighQuality() {
	...
	interp = ( idMath::Sin( (float)( GAME_TIME - startWarpTime ) / 1000 ) + 1 ) / 2.f;
