...
static int QEW_SetupPixelFormat( HDC hDC, bool zbuffer ) {
	int pixelFormat = Win_ChoosePixelFormat( hDC );
	if ( pixelFormat > 0 ) {
		if ( SetPixelFormat(hDC, pixelFormat, &win32.pfd ) == NULL) {
			common->Error( "SetPixelFormat failed." );
		}
	} else {
		common->Error( "ChoosePixelFormat failed." );
	}
...
void idGLDrawableMaterial::draw(int x, int y, int w, int h) {
	...
	if (mat) {
		...
		if (worldDirty) {
			...
			idImage *img = ( mat->GetNumStages() > 0 ) ? mat->GetStage(0)->texture.image : globalImages->defaultImage;
