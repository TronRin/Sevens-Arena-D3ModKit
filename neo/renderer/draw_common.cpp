...
void RB_SetProgramEnvironment( bool isPostProcess ) {
	...
	glProgramEnvParameter4fvARB( GL_FRAGMENT_PROGRAM_ARB, PP_CURDEPTH_RECIPR, parm );
...
void RB_STD_T_RenderShaderPasses( const drawSurf_t *surf ) {
	...
	for ( stage = 0; stage < shader->GetNumStages() ; stage++ ) {
		...
		if ( newStage ) {
			...
			glDisable( GL_FRAGMENT_PROGRAM_ARB );
			// Fixme: Hack to get around an apparent bug in ATI drivers.  Should remove as soon as it gets fixed.
			glBindProgramARB( GL_VERTEX_PROGRAM_ARB, 0 ); // FIXME: ...
				 ...
				 && !r_skipNewAmbient.GetBool() )
		{
			...
			glBindProgramARB( GL_VERTEX_PROGRAM_ARB, VPROG_SOFT_PARTICLE );
			glEnable( GL_VERTEX_PROGRAM_ARB );
			...
			glBindProgramARB( GL_FRAGMENT_PROGRAM_ARB, FPROG_SOFT_PARTICLE );
			glEnable( GL_FRAGMENT_PROGRAM_ARB );
...
#undef MY_CASE
			...
			glProgramEnvParameter4fvARB( GL_FRAGMENT_PROGRAM_ARB, PP_PARTICLE_RADIUS, parm );
			...
			glProgramEnvParameter4fvARB( GL_FRAGMENT_PROGRAM_ARB, PP_PARTICLE_COLCHAN_MASK, parm );
