...
namespace {
	...
	static idCVar imgui_numBindingColumns( "imgui_numBindingColumns", "3", CVAR_ARCHIVE|CVAR_SYSTEM|CVAR_INTEGER, "Number of columns with bindings in Settings Menu's Bindings tab", 1, 10 );
	...
	*/
	static bool IsConfirmKeyPressed( void ) {
	...
	*/
	static bool IsCancelKeyPressed( void ) {
		...
		// Note: In Idtech4, Escape opens/closes the main menu, so in here the gamepad Start button
	...
	static const char *GetGamepadStartName( void ) {
	...
	static const char *GetGamepadBindNowButtonName( void ) { // TODO: rename to confirm or sth
		return Sys_GetLocalizedJoyKeyName( K_JOY_BTN_SOUTH ); // xbox A
	}

	static const char *GetGamepadUnbindButtonName( void ) {
	...
	const char *GetKeyName( int keyNum, bool localized = true ) {
		...
		// handle scancodes separately, because ImGui uses UTF-8, while idtech4 uses ISO8859-1
	...
	static float CalcDialogButtonWidth( void ) {
	...
	static void InitBindingEntries( void ) {
	...
	*/
	static void UpdateKeyBindingsInEntries( void ) {
	...
	static void DrawBindingsMenu( void ) {
		...
		const float commandColumnWidth = ImGui::CalcTextSize( "settings menu" ).x + defFramePadding.x * 2.0f;
	...
	static CVarOption videoOptionsImmediately[] = {
		...
		CVarOption( "r_windowResizable", "Make the game window resizable", OT_BOOL ),
	...
	static void SetVideoStuffFromCVars( void ) {
	...
	static bool VideoHasResettableChanges( void ) {
		const int curMode = r_mode.GetInteger();
	...
	static bool VideoHasApplyableChanges( void ) {
	...
	static void ApplyVideoSettings( void ) {
		cmdSystem->BufferCommandText( CMD_EXEC_APPEND, "vid_restart partial\n" );
	}

	static void VideoResetChanges( void ) {
		r_mode.SetInteger( initialMode );
	...
	static void InitVideoOptionsMenu( void ) {
	...
	static void DrawVideoOptionsMenu( void ) {
		ImGui::Spacing();
	...
	static void DrawAudioOptionsMenu( void ) {
...
static bool settingsWinInitialized = false;

static void InitSettingsMenu( void ) {
	InitBindingEntries();
	...
	float defaultWidth = ImGui::CalcTextSize( "BindingsControllerVideoAudioGameOther" ).x;
...
void Com_DrawSettingsMenu( void ) {
	...
	if ( !settingsWinInitialized ) {
		// NOTE: InitSettingsMenu() must be called after/by NewFrame(), because it calls
		...
		//       Com_OpenCloseSettingsMenu() caused problems (crashes)
		InitSettingsMenu();
	}
	...
	// switching to a lower resolution, restarting the game will fix it)
...
void Com_OpenCloseSettingsMenu( bool open ) {
	if ( open ) {
		...
		settingsWinInitialized = false; // make sure it's initialized in first frame
	} else {
