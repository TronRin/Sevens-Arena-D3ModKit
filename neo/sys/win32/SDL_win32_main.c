...
#error "adapt homedir code for wchar_t!"
	...
	/* DG: use "My Documents/My Games/GAME_NAME" to write stdout.txt and stderr.txt
	 *     instead of the binary, which might not be writable */
	Win_GetHomeDir(path, sizeof(path));

	if (_stat(path, &st) == -1) {
		/* oops, "My Documents/My Games/GAME_NAME" doesn't exist - does My Games/ at least exist? */
		...
		_mkdir(path); /* create My Documents/My Games/GAME_NAME/ */
	}
	...
	{ /* DG: rename old stdout log */
		...
		wcsncat( stdoutPathBK, DIR_SEPERATOR TEXT("enginelog-old.txt"), SDL_arraysize(stdoutPath) );
