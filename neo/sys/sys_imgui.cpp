...
// the rest of the engine will only reference the "imgui" variable, while all local aspects stay hidden
idImGuiLocal				imguiLocal;
idImGui*					imgui = &imguiLocal;	// statically pointed at an idImGuiLocal

// implemented in imgui_savestyle.cpp
namespace DG {
	/*!
		writes the given ImGuiStyle to the given filename (opened with fopen())
		@param[in] style The ImGuiStyle to write.
		@param[in] filename The Style file name to write.
		@param[out] boolean returns true on success, false if opening the file failed.
	*/
	...
	/*!
		...
		@param[in] style The ImGuiStyle to read.
		@param[in] filename The Style file name to read.
		@param[out] returns true on success, false if opening the file failed
	*/
...
idImGuiLocal::idImGuiLocal() :
	keybindModeEnabled( false ),
	imgui_initialized( false ),
	sdlWindow( NULL ),
	haveNewFrame( false ),
	openImguiWindows( 0 ),
	hadKeyDownEvent( false ),
	warningOverlayStartTime( -100.0 ) {

}

bool idImGuiLocal::Init( void *_sdlWindow, void *sdlGlContext ) {
	...
	SetImGuiStyle( idImGuiStyle::Dhewm3 );
...
void idImGuiLocal::Shutdown( void ) {
...
void idImGuiLocal::OpenWindow( idImGuiWindow win ) {
...
bool idImGuiLocal::ProcessEvent( const void *sdlEvent ) {
...
bool idImGuiLocal::ShouldShowCursor( void ) {
	if ( sessLocal.GetActiveMenu() == nullptr ) {
		// when ingame, render the ImGui/SDL/system cursor if an ImGui window is open
		// because the engine does *not* render its own cursor outside ImGui windows.
		// additionally, only show it if an ImGui window has focus - this allows you
		// to click outside the ImGui window to give the engine focus and look around.
		...
		// or in one of the black bars where Engine's cursor isn't rendered in
		// non 4:3 resolutions
		if ( openImguiWindows == 0 ) {
		...
		// if scaling Engine menus to 4:3 is enabled and the cursor is currently
		// in a black bar (Engine cursor is not drawn there), show the ImGui cursor
...
void idImGuiLocal::NewFrame( void ) {
	...
	if ( openImguiWindows & WINDOW_SETTINGS ) {
		Com_DrawSettingsMenu();
	}

	if ( openImguiWindows & WINDOW_OVERLAYS ) {
		Com_DrawOverlays();
	}

	if ( openImguiWindows & WINDOW_DEMO ) {
		bool show_demo_window = true;
		ImGui::ShowDemoWindow( &show_demo_window );
		if ( !show_demo_window ) {
			CloseWindow( WINDOW_DEMO );
		}
	}
}

void idImGuiLocal::EndFrame( void ) {
	...
	// Idtech4 uses the OpenGL ARB shader extensions, for most things it renders.
	// disable those shaders, the OpenGL classic integration of ImGui doesn't use shaders
	glDisable( GL_VERTEX_PROGRAM_ARB );
	glDisable( GL_FRAGMENT_PROGRAM_ARB );

	// Idtech4 uses OpenGL's ARB_vertex_buffer_object extension to use VBOs on the GPU
...
void idImGuiLocal::SetKeyBindMode( bool enable ) {
...
float idImGuiLocal::GetDefaultDPI( void ) {
...
float idImGuiLocal::GetDefaultScale( void ) {
...
float idImGuiLocal::GetScale( void ) {
...
void idImGuiLocal::SetScale( float scale ) {
	imgui_scale.SetFloat( scale );
}

void idImGuiLocal::UpdateWarningOverlay( void ) {
...
void idImGuiLocal::ShowWarningOverlay( const char *text ) {
	warningOverlayText = text;
	warningOverlayStartTime = ImGui::GetTime();
	warningOverlayStartPos = ImGui::GetMousePos();
}

idStr idImGuiLocal::GetUserStyleFilename( void ) {
	idStr ret( cvarSystem->GetCVarString( "fs_configpath" ) );
	ret += "/user.imstyle";
	return ret;
}

ImGuiStyle idImGuiLocal::GetImGuiStyle( idImGuiStyle d3style ) {
	ImGuiStyle style; // default style
	if ( d3style == idImGuiStyle::Dhewm3 ) {
	...
	} else if ( d3style == idImGuiStyle::User ) {
		style = userStyle;
	} else {
		assert( d3style == idImGuiStyle::ImGui_Default && "invalid/unknown style" );
		ImGui::StyleColorsDark( &style );
	}
	return style;
}

void idImGuiLocal::SetImGuiStyle( idImGuiStyle d3style ) {
	ImGui::GetStyle() = GetImGuiStyle( d3style );
}

void idImGuiLocal::SetDhewm3StyleColors( ImGuiStyle *dst ) {
	if ( dst == nullptr ) {
		dst = &ImGui::GetStyle();
	}
...
void idImGuiLocal::SetUserStyleColors( void ) {
...
bool idImGuiLocal::WriteUserStyle( void ) {
...
void idImGuiLocal::CopyCurrentStyle( bool onlyChanges ) {
	ImGuiStyle refStyle = GetImGuiStyle( idImGuiStyle::ImGui_Default );
